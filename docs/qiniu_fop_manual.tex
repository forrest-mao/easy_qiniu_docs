...
\begin{sample}
  \caption{获取图片基本信息}
    ...
    \qpar{$\Rightarrow$}
    \qsamplelink{\qurl{http://newdocs.qiniudn.com/gogopher.jpg?imageInfo}}
...
\begin{sample}
  \caption{获取图片EXIF信息}
    ...
    \qsamplelink{\qurl{http://newdocs.qiniudn.com/gogopher.jpg}}
    \qpar{$\Rightarrow$}
    \qsamplelink{\qurl{http://newdocs.qiniudn.com/gogopher.jpg?exif}}
...
\qpar{在图片下载URL后附加\qqbox{imageView}指示符（区分大小写）并指定缩略操作参数，可以生成该图片的缩略图。}
\qpar{调用接口：}
...
\qpar{七牛云处理服务支持添加图片水印或文字水印，使用方法为在下载URL后添加\qqbox{watermark}指示符（区分大小写）并附加相关参数。}
\qpar{图片水印调用接口：}
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize]
<URL>?watermark/1
...
\qpar{水印操作参数说明表：}
\qtable{
\begin{tabular}[t]{|l|p{12em}|p{10em}|}
\hline
参数名称 & 说明 & 示例 \\
\hline
...
\begin{sample}
  \caption{文字水印}
    \qsamplelink{\qurl{http://newdocs.qiniudn.com/gogopher.jpg}}
    \qpar{$\Rightarrow$}
    \qsamplelink{\qurl{http://newdocs.qiniudn.com/gogopher.jpg?watermark/2/text/5LiD54mb5LqR5a2Y5YKo/font/U2ltSGVp/fontsize/600/fill/Izc0RDJGRg==/gravity/North}}
...
\qpar{七牛云服务支持为图片处理规格设定别名，避免往下载URL中引入QueryString字符串（即问号后的部分），从而规避可能引发的各种使用不便。}
\qpar{每个资源空间（Bucket）可以设定多个别名和分隔符，以提供灵活的使用方式。}
\qpar{七牛\href{https://portal.qiniu.com}{开发者后台}或\href{http://docs.qiniu.com/tools/v6/qboxrsctl.html}{开发者工具（qboxrsctl）}来都可以定制别名。前文描述的各种处理均支持使用别名。}
...
qboxrsctl style <bucket> small.jpg imageView/1/w/200/h/200 # 设定别名
qboxrsctl separator <bucket> -                             # 设定分隔符
    \end{verbatim}

    \qsamplelink{\qurl{http://newdocs.qiniudn.com/gogopher.jpg?imageView/1/w/200/h/200}}
    \qpar{$=$}
    {\qurl{http://newdocs.qiniudn.com/gogopher.jpg-small.jpg}\vspace{-0.8em}}
\footnotesize
\begin{verbatim}
...
\qpar{通常图片处理是在下载时执行，但可能增加下载延迟（如图片很大，处理时间会变长）。为避免增加无意义的下载延迟，可以使用预转服务，在图片上传成功后即刻处理。}
\qpar{预转属于异步操作，上传成功完成后自动触发服务器处理，无须等待其完成，进一步提升使用体验。}
\qpar{使用方法请参考\href{http://docs.qiniu.com/api/v6/put.html\#uploadToken-asyncOps}{uploadToken的asyncOps字段。}}
...
\begin{itemize}
  \item 图片处理执行时机？\par
  \qpar{通常而言，图片处理是在下载时执行，但可以使用预转功能将执行时机提前到图片上传成功时。}

  \item 图片处理结果是否计入存储用量？\par
  \qpar{图片处理结果将被缓存，不计入存储用量中。}
\end{itemize}
