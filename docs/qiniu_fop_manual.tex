...
\qpara{在图片下载URL后附加\qqbox{imageInfo}指示符（区分大小写），可以获取JSON格式的图片基本信息，包括图片格式、宽高值（单位：像素）、色彩模型。\textcolor{red}{注意：缩略图不支持该方法。}}
...
\qpara{获取方式为在图片下载URL后附加\qqbox{exif}指示符（区分大小写）。\textcolor{red}{注意：缩略图不支持该方法。}}
